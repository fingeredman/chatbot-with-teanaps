...
---
## Notice! 
> - 카카오톡 채널과 `TEANAPS`를 활용한 챗봇 구축 실무/연구 프로젝트 **지원이 필요하신 분들** 또는 **사례 공유가 가능하신 분**의 연락을 기다립니다.     
...
- `TEANAPS Chat-bot` 시스템 구성도
  ...
  - 의도분류기 관리자 (NLU Handler)
    ...
    - 의도분류기를 `직렬 방식` 또는 `병렬 방식`으로 배치하는 순서는 상황에 따라 임의로 정할 수 있습니다. `TEANAPS Chat-bot`은 카카오톡 인터페이스 연동이 용이하도록 `Kakao i Open Builder` 의도분류기를 `직렬 방식`의 가장 앞에 배치하고, `Google Dialogflow`를 포함한 `지도학습 기반` 의도분류기를 뒤에 `병렬 방식`으로 배치합니다. `Kakao i Open Builder` 의도분류기에서 의도를 분류하지 못하고(의도분류 결과가 임계값을 넘지 못하는 경우) `Fallback`을 발생하는 경우, 뒤에 `병렬 방식` 의도분류기 중 선택된 의도분류기에서 다시한번 의도를 분류한 후 답변을 제공합니다.  
      ...
      1. `직렬 방식` 의도분류기 배치 : 동일한 학습데이터로 학습된 의도분류기를 직렬로 여러개 배치합니다. 앞선 의도분류기에서 분류를 정확하게 하지 못한 경우, 다음번 의도분류기에서 다시한번 분류합니다. 앞선 의도분류기의 성능을 뒤에 배치된 의도분류기가 보완하는 역할을 합니다. 앞선 의도분류기에서 의도분류에 성공한 경우, 다음 의도분류기를 분류를 수행하지 않습니다. 따라서 분류정확도 및 커버리지가 높은 의도분류기를 앞에 배치하는 것이 효율적입니다.
      2. `병렬 방식 - 스위칭 방식` 의도분류기 배치 : 경우에 따라 인텐트 별로 특정 의도분류 모델에서 분류가 잘 되는 경우가 있습니다 (단순질의, 일상대화 등). 이런 경우, 병렬로 배치된 복수개의 의도분류기 앞에 셀렉터(또는 스위치)를 두고 인텐트 그룹을 한번 더 구분하고 해당 그룹에 맞는 의도분류기로 의도분류 할 수 있도록 합니다.
      3. `병렬 방식 - 앙상블 방식` 의도분류기 배치 : 여러개의 모델을 조합하여 하나의 모델을 활용할 때 보다 더 좋은 성능을 내도록 구성한 모델을 `앙상블(ensenble) 모델`이라고 합니다. 의도분류 모델도 여러개 알고리즘을 조합하여 하나의 모델을 활용할 때 보다 더 좋은 성능을 내도록 구성할 수 있습니다.
...
- 처리과정 및 역할
    ...
    - ① 질문
      ...
      > | ①-3    | 사용자가 입력한 대화 문장에 대해 답변을 위한 정보가 부족한 경우, 추가적인 개체명 정보를 사용자에게 요청합니다.|
    ...
    - ② 의도파악
      ...
      > | ②-3      | 질문의 의도를 명확하게 파악하기 위해 질문에 포함된 개체명(시간정보, 인물명, 장소명 등)을 추출합니다. | 
      > | ②-4      | 질문에서 추출된 개체명 정보가 답변하기에 충분한지 확인합니다. 일반적으로 챗봇에서 사용자 메시지에서 의도파악에 필요한 개체명 정보를 추출하는 과정을 슬롯필링(Slot-filling)이라고 합니다. (의도가 맛집 정보를 물어보는 경우, 장소명 포함여부 확인) | 
      > |          | 개체명 정보가 충분하면 [③-3] 과정으로 넘어가고, 충분하지 않으면 [①-3] 과정으로 넘어가 필요한 개체명 정보를 사용자에게 요청합니다. |
